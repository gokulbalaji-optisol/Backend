...
export default class PaymentController {
    ...
    const createOrder = async (cust, datas) => {
      ...
      try {
        cartItems.map((item) => {
          ...
          const subtotal =
            parseInt(item.quantity) * parseInt(item.book_id.price);
  ...
  static async listAllCoupon(req: Request, res: Response) {
  ...
  static async listAllPromoCode(req: Request, res: Response) {
    const { limit, coupon } = req.query;
    const promocode = await stripe.promotionCodes.list({
      coupon: coupon,
      limit: limit,
    });
    return res.status(200).send(promocode.data);
  }
  static async createPromoCode(req: Request, res: Response) {
    let { coupon, promocode } = req.body;
    try {
      const promotionCode = await stripe.promotionCodes.create({
        coupon: coupon,
        code: promocode,
      ...
      res.send({
        message: "promotion code create successfully ",
        promotionCode,
      });
    } catch (err) {
      console.log("Promo code err", err);
      res.status(400).send({
        message: "promotion code creation failed ",
      });
    }
  }
  ...
  static async deleteCoupon(req: Request, res: Response) {
    ...
    let resp;
    try {
      resp = await stripe.coupons.del(coupon);
    } catch (err) {
      resp = err;
      console.log(err);
      return res
        .status(400)
        .send({ data: err, message: "coupon deleted failed" });
    }
    return res.send({ data: resp, message: "Coupon Deleted Successfully" });
  }

  static async getPromoCodeByCouponId(req: Request, res: Response) {
    const { promocode } = req.query;
    const response = await stripe.promotionCodes.retrieve(promocode);
  ...
  static async updatePromoCode(req: Request, res: Response) {
    ...
    try {
      ...
      res.send({
        message: "promotion code status changed successfully ",
        promotionCode,
      });
    } catch (err) {
      console.log("Promo code err", err);
